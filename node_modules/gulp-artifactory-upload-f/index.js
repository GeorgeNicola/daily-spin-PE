...
var PLUGIN_NAME = 'gulp-artifactory-upload-f';

var parseResponse = function( responseData, callback ) {

	gutil.log( responseData );
	callback();
...
var processFile = function( url, options, stream, callback ) {
	...
	var reqx = request.del( url );
	if ( options.username && options.password ) {
		reqx.auth(options.username, options.password, true);
	}
	reqx.on( 'error', function( err ) {
		if ( !called ) {
			called = true;
			callback( err );
		}
